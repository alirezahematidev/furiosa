...
function generator(input: SwaggerJson, config: Config): { code: string; hooks: string; type: string } {
  ...
  const includes = (config.includes || []).map((pattern) => new RegExp(pattern));
  const excludes = (config.excludes || []).map((pattern) => new RegExp(pattern));

  function getModifiedPathValue(endPoint: string, value: PathItem) {
    return Object.entries(value).filter(([method, options]: [string, SwaggerRequest]) => {
      const { operationId } = options;

      const serviceName = generateServiceName(endPoint, method, operationId, config);

      const matchesInclude = !includes.length || includes.some((regex) => regex.test(serviceName));

      const matchesExclude = excludes.some((regex) => regex.test(serviceName));
