...
export class _Internal<T extends TData> {
  ...
  private trigger<TPath extends string>(field: TPath, dispose: ObservableListenerDispose | null) {
    ...
    return async ({ changes, value }: ListenerParams) => {
      ...
      const errors = await parseSchemaValidator<T>({ data, keys, validator: this.validator, getFields: this.getFields.bind(this) });
  ...
  public get<TPath extends string>(field?: TPath | Array<TPath>, peekable = false) {
    ...
    if (Array.isArray(field)) return [...new Set(field)].reduce((p, q) => ({ ...p, [this.chunk(q)]: deepObserve(this.table$, q)[method]() }), {});
  ...
  public set<TPath extends string>(field: TPath, value: any): void {
    ...
    if (this.revalidateMode === "change") {
      this.dispose = current.onChange(this.trigger(field, this.dispose), { initial: true, immediate: true });
  ...
  public array(params: FieldArrayParams<T>) {
    ...
    const methods = { append, prepend, replace, insert, move, remove, swap, update, _select };
