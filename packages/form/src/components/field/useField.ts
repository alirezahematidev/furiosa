import { cloneElement, startTransition, useCallback, useMemo, useRef } from "react";
...
export function useField<T extends TData, TPath extends DeepArrayPath<T>>(props: FieldProps<T, TPath>) {
  ...
  const hook = useMemo(() => new Hook<T>(api), [api]);
  ...
  useFieldRegistry({ hook, name, binding$, shouldUnregister: _connect || shouldUnregister });

  const onChange = useCallback((event: any) => startTransition(() => hook.set(name, getEventValue(event))), [hook, name]);
  ...
  return () => {
    ...
    return cloneElement(render, { ...options, ...render.props });
  };
}
