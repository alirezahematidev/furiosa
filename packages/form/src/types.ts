...
export type TransformFieldValues<T extends TData, TPaths extends Array<DeepArrayPath<T>>> = { readonly [K in Underscore<TPaths[number]>]: DeepArrayPathValue<T, Chain<K>> };
...
export type PathValue<T, P extends string, U = never> = SplitPath<P> extends [infer K, infer R]
  ? K extends keyof T
    ? R extends Path<T[K]>
...
type ConnectorProps<T extends TData, TPath extends DeepArrayPath<T>> = { _connect: boolean; disconnect?: DisconnectFunction<T, TPath> };
