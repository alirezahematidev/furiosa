...
export class FormApi<T extends TData> {
  ...
  constructor(options: FormOptions<T>) {
    ...
    this.status$ = observable<FormStatus>({ isSubmitting: false, isValidating: false, isLoading: false });
  ...
  static createConnector<U extends TData>() {
    const fromApi = new FormApi<U>({ defaultValues: {} as U, revalidateMode: "change" });
